...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const fetchCart = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error && 'response' in error 
        ? (error as any).response?.data?.message || "Failed to fetch cart"
  ...
  const addToCart = async (productId: number, quantity: number = 1) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error && 'response' in error 
        ? (error as any).response?.data?.message || "Failed to add item to cart"
  ...
  const removeFromCart = async (itemId: number) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error && 'response' in error 
        ? (error as any).response?.data?.message || "Failed to remove item from cart"
  ...
  const clearCart = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error && 'response' in error 
        ? (error as any).response?.data?.message || "Failed to clear cart"
